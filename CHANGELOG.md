...
- **`norn get --col .body` / `.raw` now load what they request without needing a separate flag.** A requested heavy facet self-loads: `get <doc> --col .body` (without `--body`) used to show nothing because the body only populated when `--body` was passed — it now loads the body whenever `--body` *or* `.body` is requested, and `.raw` does its disk read whenever `.raw` is requested. Both stay opt-in: neither appears in the default (no-`--col`) view.
