...
- **The `norn get --body` flag is removed.** Body content is now reached through the unified `--col` vocabulary that `get` already shares with `find`: `--all-cols` for the full structured dump (which self-loads the body) or `--col .body` for the body on its own. The standalone `--body` boolean was a pre-unification artifact — it predated `.body` being a first-class facet, and once `.body` self-loads on request (see Added) the flag is redundant. **Breaking (pre-1.0):** `get <doc> --body` now errors as an unknown flag; use `get <doc> --all-cols` or `get <doc> --col .body`. This is the last piece of the find/get shared output contract — body is a column, reached the same way every other facet is.
- **`norn get` now offers the same output formats as `norn find`: `records`, `paths`, `json`, `jsonl`.** Previously `get` had only `text` and `json`. The `text` format is **renamed to `records`** (it always *was* the vertical key-value record-block primitive `find` calls `records`), and `get` gains `paths` (one document path per line; `--col` is inert and warns, exactly as on `find`) and `jsonl` (one JSON record object per line — the streaming sibling of the `json` array). **Breaking (pre-1.0):** `get --format text` → `get --format records`. The default is now `records`. This is step two of giving `find` and `get` one shared output contract over their two different selection front-ends.
- **`norn get`'s default `records` view renders each frontmatter field as its own labeled line, matching `norn find`.** Previously `get`'s default emitted frontmatter as a single consolidated `frontmatter` block (all keys inline under one label); `find`'s `records` emitted each key as its own labeled line. Under one shared contract a bare field is a column, so both now render frontmatter fields as individual labeled lines at rest. **Breaking (pre-1.0):** the consolidated block is no longer the default — request it explicitly with `get --col .frontmatter` (which still emits the whole block). `json`/`jsonl` are unaffected (frontmatter was already an object under the `frontmatter` key in both).
- **`norn get --col` now selects frontmatter fields by bare name, like `norn find`.** Previously `get --col` accepted only a fixed set of structural section names (`frontmatter`, `headings`, `body`, the link sets) and rejected anything else — so `get <doc> --col status` warned `unknown --col field 'status'` and showed nothing, even though `find --col status` projects that field. The two commands now share one `--col` model: **bare names are frontmatter fields** (`get <doc> --col status,title` projects just those, identical to `find`), and the structural facets are **dot-prefixed** — `.path`, `.frontmatter` (the whole block), `.headings`, `.body`, `.outgoing_links`, `.unresolved_links`, `.incoming_links`. The leading dot keeps a frontmatter key named e.g. `body` from colliding with the body facet. **Breaking (pre-1.0):** existing `get --col <section>` calls must add the dot — `get --col body` → `get --col .body`, `get --col frontmatter` → `get --col .frontmatter`, etc. With no `--col`, the full default dump is still emitted (its `records`-view frontmatter rendering changes — see the format entry above). (The nested/dotted *frontmatter*-key half of the addressing model — `--col schema.version` — remains future work; this ships the top-level field projection + the facet split.)
